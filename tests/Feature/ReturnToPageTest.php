...
class ReturnToPageTest extends TestCase
{
    use RefreshDatabase;

    public function test_return_to_page(): void
    {
        ...
        $response = $this->actingAs($user)
            ->get('returnToPage', ['page' => $page, 'keyword' => fake()->randomElement($keywords)])
            ->assertStatus(200)
            ->assertSeeText(["Hello {$user->name}", "Rush", "7.716"]);
        // assert that the movie above is shown on the page
